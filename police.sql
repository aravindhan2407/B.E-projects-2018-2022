...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 13, 2020 at 08:59 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `delcomp` ()  MODIFIES SQL DATA
BEGIN
declare cnt int(1);
declare c int(3);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `delreq` ()  NO SQL
BEGIN
declare cnt int(1);
declare c int(3);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `deltraf` ()  MODIFIES SQL DATA
BEGIN
declare cnt int(1);
declare c int(3);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `priorityforcomplaint` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `priorityforrequest` ()  NO SQL
BEGIN
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `compstatus` (`c_id` INT(3) UNSIGNED) RETURNS VARCHAR(20) CHARSET latin1 READS SQL DATA
BEGIN
declare descr varchar(20);
declare st int(1);
...
CREATE DEFINER=`root`@`localhost` FUNCTION `reqstatus` (`r_id` INT(4)) RETURNS VARCHAR(20) CHARSET latin1 READS SQL DATA
BEGIN
declare descr varchar(20);
declare st int(1);
...
CREATE DEFINER=`root`@`localhost` FUNCTION `trafstatus` (`i_id` INT(4)) RETURNS VARCHAR(20) CHARSET latin1 NO SQL
BEGIN
declare descr varchar(20);
declare st int(1);
...
INSERT INTO `complaint` (`COMPLAINT_ID`, `NAME`, `AREA`, `TIME`, `PHONE_NUMBER`, `PRIORITY`, `STATUS`, `DESCRIPTION`) VALUES
(1, 'yogeshwar', 3, '2020-04-13 05:29:07', 23456, 10, 2, 'MURDER'),
(2, 'war', 2, '2020-04-13 05:32:08', 4314123, 0, 1, 'war'),
(3, 'Max', 3, '2020-04-13 05:41:38', 2147483647, NULL, 1, 'Murder');

--
-- Triggers `complaint`
--
DELIMITER $$
CREATE TRIGGER `COMPLAINT_AFTER_INSERT` AFTER INSERT ON `complaint` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `COMPLAINT_BEFORE_DELETE` BEFORE DELETE ON `complaint` FOR EACH ROW BEGIN
...
CREATE TABLE `involved_in_1` (
  `POLICE_OFFICER_POLICE_ID` int(11) NOT NULL,
  `REQUEST_REQUEST_ID` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `involved_in_1`
--

INSERT INTO `involved_in_1` (`POLICE_OFFICER_POLICE_ID`, `REQUEST_REQUEST_ID`) VALUES
(611, 1);
...
--
DELIMITER $$
CREATE TRIGGER `INVOLVED_IN_1_AFTER_DELETE` AFTER DELETE ON `involved_in_1` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `INVOLVED_IN_1_AFTER_INSERT` AFTER INSERT ON `involved_in_1` FOR EACH ROW BEGIN
...
CREATE TABLE `involved_in_2` (
  `POLICE_OFFICER_POLICE_ID` int(11) NOT NULL,
  `COMPLAINT_ID` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `involved_in_2`
--

INSERT INTO `involved_in_2` (`POLICE_OFFICER_POLICE_ID`, `COMPLAINT_ID`) VALUES
(612, 1),
(618, 2),
(612, 3);
...
--
DELIMITER $$
CREATE TRIGGER `INVOLVED_IN_2_AFTER_DELETE` AFTER DELETE ON `involved_in_2` FOR EACH ROW BEGIN
update police_officer set status=1 where police_id = old.police_officer_police_id;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `INVOLVED_IN_2_AFTER_INSERT` AFTER INSERT ON `involved_in_2` FOR EACH ROW BEGIN
update police_officer set status=0 where police_id = new.police_officer_police_id;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `INVOLVED_IN_3_AFTER_DELETE` AFTER DELETE ON `involved_in_3` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `INVOLVED_IN_3_AFTER_INSERT` AFTER INSERT ON `involved_in_3` FOR EACH ROW BEGIN
...
INSERT INTO `request` (`REQUEST_ID`, `NAME`, `AREA`, `TIME`, `PHONE_NUMBER`, `PRIORITY`, `STATUS`, `DESCRIPTION`) VALUES
(1, 'qw', 1, '2020-04-09 14:14:16', 32321, 0, 2, 'eq');

--
-- Triggers `request`
--
DELIMITER $$
CREATE TRIGGER `REQUEST_AFTER_DELETE` AFTER DELETE ON `request` FOR EACH ROW BEGIN
...
(5, 'yogesh', 'babu', 'Male', 'yogeshwarbabu7@gmail.com', '81dc9bdb52d04dc20036dbd8313ed055');
...
--
DELIMITER $$
CREATE TRIGGER `TRAFFIC_ISSUE_AFTER_INSERT` AFTER INSERT ON `traffic_issue` FOR EACH ROW BEGIN
declare tmplft integer;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRAFFIC_ISSUE_BEFORE_DELETE` BEFORE DELETE ON `traffic_issue` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `TRAFFIC_SIGNAL_AFTER_INSERT` AFTER INSERT ON `traffic_signal` FOR EACH ROW BEGIN
declare tmplft integer;
...
ALTER TABLE `complaint`
  ...
  ADD KEY `fk_COMPLAINT_STATUS1_idx` (`STATUS`),
  ADD KEY `ADDRESS_idx` (`AREA`);

--
-- Indexes for table `involved_in_1`
--
ALTER TABLE `involved_in_1`
  ADD KEY `fk_INVOLVED_IN_POLICE_OFFICER1_idx` (`POLICE_OFFICER_POLICE_ID`),
  ADD KEY `fk_INVOLVED_IN_REQUEST1_idx` (`REQUEST_REQUEST_ID`);

--
-- Indexes for table `involved_in_2`
--
ALTER TABLE `involved_in_2`
  ADD KEY `COMPLAINT_idx` (`COMPLAINT_ID`),
  ADD KEY `POLICE_idx` (`POLICE_OFFICER_POLICE_ID`);

--
-- Indexes for table `involved_in_3`
--
ALTER TABLE `involved_in_3`
  ADD KEY `POLICE_ID_idx` (`POLICE_ID`),
  ADD KEY `ISSUE_idx` (`ISSUE_ID`);
...
ALTER TABLE `police_officer`
  ...
  ADD UNIQUE KEY `POLICE_ID_UNIQUE` (`POLICE_ID`),
  ADD KEY `fk_POLICE_OFFICER_POLICE_STATION2_idx` (`STATION`),
  ADD KEY `fk_POLICE_OFFICER_OFFICER_STATUS2_idx` (`STATUS`);

--
-- Indexes for table `police_station`
--
ALTER TABLE `police_station`
  ADD PRIMARY KEY (`STATION_NAME`,`STATION_AREA`),
  ADD UNIQUE KEY `STATION_NAME_UNIQUE` (`STATION_NAME`),
  ADD KEY `fk_POLICE_STATION_AREAS1_idx` (`STATION_AREA`);
...
ALTER TABLE `request`
  ...
  ADD UNIQUE KEY `COMPLAINT_ID_UNIQUE` (`REQUEST_ID`),
  ADD KEY `fk_REQUEST_STATUS2_idx` (`STATUS`),
  ADD KEY `ADDRESS_idx` (`AREA`);
...
ALTER TABLE `status`
  ADD PRIMARY KEY (`STATUS_CODE`),
  ADD UNIQUE KEY `STATUS_CODE_UNIQUE` (`STATUS_CODE`);
...
ALTER TABLE `traffic_issue`
  ...
  ADD UNIQUE KEY `COMPLAINT_ID_UNIQUE` (`ISSUE_ID`),
  ADD KEY `fk_TRAFFIC_ISSUE_TRAFFIC_STATUS2_idx` (`STATUS`);
...
ALTER TABLE `traffic_signal`
  ...
  ADD UNIQUE KEY `SIGNAL_ID_UNIQUE` (`SIGNAL_ID`),
